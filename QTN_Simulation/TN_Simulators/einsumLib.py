...
def einsumForCircuit(tensorInputString, lines, circuit, strategy):
    s = ""
    s += tensorInputString
    index = getChars(strategy).index(tensorInputString[-1]) + 1
    for i in range(len(circuit)):
        (gate,qubits) = circuit[i]
        gateInS = ""
        gateOutS = ""
        for line in qubits:
            gateInS += lines[line]
...
def apply_Gate_On_Tensor(tensor, gate = np.array([]), qubits=[], baseEinsumStrategy = np.einsum, es_str: str = None):
    if(es_str != None):
